...
if (as.logical(Sys.getenv("NETWORK_UP", unset = TRUE)) &
  "germinationmetrics" %in% available.packages()[, 1]) {
...
rver <- ifelse(test = gsub("(.\\.)(\\d+)(\\..)", "", getNamespaceVersion("germinationmetrics")) == "",
  ...
  rver <- getNamespaceVersion("germinationmetrics")
  print("Unable to fetch release version from CRAN.")
}
...
#### `GermIndex() `
```{r, runCode, code = getCode("GermIndex", "germinationmetrics")}
```

#### `EmergenceRateIndex() `
```{r, runCode, code = getCode("EmergenceRateIndex", "germinationmetrics")}
```
...
```{r, runCode, code = getCode("FourPHFfit.bulk", "germinationmetrics")}
```

Multiple fitted curves generated in batch can also be plotted.

```{r, runCode, fig.width = 6, fig.height = 4, code = getCode("plot.FourPHFfit.bulk", "germinationmetrics")}
```
