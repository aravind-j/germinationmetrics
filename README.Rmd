...
###### Version : [`r getNamespaceVersion("germinationmetrics")`](https://aravind-j.github.io/germinationmetrics/articles/Introduction.html#version-history); Copyright (C) 2017-`r  format(Sys.Date(), "%Y")`: [ICAR-NBPGR](https://nbpgr.org.in/); License: [GPL2|GPL-3](https://www.r-project.org/Licenses/)
...
url_exists <- function(url) {
  ...
  tryCatch({
    fetch <- curl::curl_fetch_memory(url, handle = h)
    status <- fetch$status_code
    status == 200 # Strict
    # status >= 200 && status < 400 # Lenient
  }, error = function(e) {
    FALSE
  })
}

if (url_exists(chkurl)) {
